...
pair<Mat, Mat> createHistogramAndCDF(const Mat& channel) {
...
void findAlphaThresholds(const Mat& cdf, double& alpha1, double& alpha2, double lower_percent, double upper_percent) {
...
int main() {
    ...
    Mat srcImage = imread("../christmas.png");
    if (srcImage.empty()) {
    ...
    Mat b_hist, g_hist, r_hist, b_cdf, g_cdf, r_cdf;
    tie(b_hist, b_cdf) = createHistogramAndCDF(bgr_planes[0]);
    tie(g_hist, g_cdf) = createHistogramAndCDF(bgr_planes[1]);
    tie(r_hist, r_cdf) = createHistogramAndCDF(bgr_planes[2]);
    ...
    cout << "Blue channel: alpha1 = " << alpha1_b << ", alpha2 = " << alpha2_b << endl;
    cout << "Green channel: alpha1 = " << alpha1_g << ", alpha2 = " << alpha2_g << endl;
    cout << "Red channel: alpha1 = " << alpha1_r << ", alpha2 = " << alpha2_r << endl;

    // Создание изображения для совмещения всего
    Mat combinedImage(512, 512, CV_8UC3, Scalar(255,255,255));

    // Помещение исходного изображения в левый верхний угол
    Mat topLeftROI = combinedImage(Rect(0, 0, 256, 256));
    resize(srcImage, topLeftROI, topLeftROI.size(), 0, 0, INTER_LINEAR);

    // Помещение гистограмм в оставшиеся области
    Mat topRightROI = combinedImage(Rect(256, 0, 256, 256));
    cvtColor(r_hist, topRightROI, COLOR_GRAY2BGR);

    Mat bottomLeftROI = combinedImage(Rect(0, 256, 256, 256));
    cvtColor(g_hist, bottomLeftROI, COLOR_GRAY2BGR);

    Mat bottomRightROI = combinedImage(Rect(256, 256, 256, 256));
    cvtColor(b_hist, bottomRightROI, COLOR_GRAY2BGR);
    ...
    Mat contrasted_b, contrasted_g, contrasted_r;
    ...
    vector<Mat> contrasted_planes = {contrasted_b, contrasted_g, contrasted_r};
